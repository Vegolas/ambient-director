...
// UI copies of the /assistant + /setup/anthropic wire shapes (duplicated by hand per the project's
...
// Mutable class — the Settings form binds the key + model inputs straight to it (like SpotifyConfigDto).
// The API never echoes the key, so ApiKey stays "" on load; the user types it to (re)configure.
