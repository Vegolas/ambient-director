...
// DurationMs is the file's natural length in ms. The server uses null (not yet measured) and 0 (tried,
// couldn't measure) as "unknown" sentinels — read it through NaturalMs, which folds both to null.
public record SoundDto(string Id, string Name, string Category, double Volume, bool Loop, int? DurationMs = null)
{
